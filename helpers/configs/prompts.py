...
- The attachment is the product specification sheet for this sensor model. 
- You are a sensor security expert.Please extract all information that may be related to sensor vulnerability, the more comprehensive and concise the better, avoiding redundancy.
- Please extract the information() into the following markdown format:
...
prompt3_1_1 = """#### Sensor Vulnerability Mechanism Taxonomy
    1. Saturation Effect
    2. Nonlinearity
    3. Non-ideal Cutoff
    4. Aliasing Effect
...
| 2   | **Antenna Effect**                  | **Interconnect (PCB Traces / Power Lines)** | Conductive traces and wiring unintentionally act as antennas under high-frequency electromagnetic radiation. The coupled EM energy is converted into electrical interference, forming an **Electromagnetic Cross-Field vulnerability** at the interconnect level.                                                                |
...
prompt3_5 = """
## Requirements:
- Considering one by one whether sound, light, and electromagnetic signals can successfully attack this sensor.And Photoacoustic Effect and Photoelectric Effect can exist at the same time
...
"""
            ...
            # **After listing all vulnerability mechanisms, include a reference section:**
            # ## References:
            # - **Group and categorize all references by their original source file names** (e.g., using subheadings like ` [Original_Filename.pdf],[Original_sentences]`).
            # - Number each reference globally in the order it is first cited ([1], [2], ...).
            # - For each reference under its respective source file category, you must provide:**Original File Name** and **Original Sentences**
            # - Every in-text citation [X] in your analysis should correspond to a numbered entry in your reference list.
...
prompt2_1="""There are various vulnerabilities in the components of sensors, which we can refer to as sensor vulnerabilities. The knowledge below can be used as a reference for vulnerability testing
  ...
  (3) Classification:
  | Vulnerability Subtype | Components Potentially Affected | Possible Mechanisms |
  |----------------------|--------------------------------|----------------------|
  | {Sensor Component Expected Modality}-Amplitude Out-of-Range | transducers; circuits | Occurs when the attack signal's amplitude exceeds the component's design limit, leading to the **saturation effect**. For transducers, this is limited by material constraints; for circuits, it is caused by supply voltage limitations. |
  | {Sensor Component Expected Modality}-Frequency Out-of-Range | filters; ADCs; signal conditioning circuits | Occurs when the attack signal's frequency exceeds the intended bandwidth. This impacts output through **nonlinearity** (e.g., inter-modulation distortion), **non-ideal cutoff** of filters, or **aliasing effects** in ADCs. |
  ...
  (2) Naming Convention:Use {Attack Modality at THIS component} Cross-Field. 
  (3) Classification :
  | Vulnerability Subtype | Components Potentially Affected | Possible Mechanisms |
  |----------------------|--------------------------------|----------------------|
  | Acoustic Cross-Field | transducers (especially MEMS structures) | Acoustic waves near the **resonance frequency** of the mechanical sensitive element induce high-intensity internal interference. |
  | Optical Cross-Field | transducers | Photoacoustic Effect: Light energy is absorbed by the **sensitive element's diaphragm**, converting into mechanical vibrations. Photoelectric Effect: Light liberates electrons from **conductive elements** (like MEMS barometers) or **ASIC transistors**, inducing unintended currents. |
  | Electromagnetic Cross-Field | signal conditioning circuits (amplifiers, ADCs); wiring | Conductive elements act as unintended antennas via the **antenna effect**, coupling EM radiation into the electrical signal path. |
  | Mechanical Cross-Field | acoustic transducers (diaphragms) | Resonance effect when vibrations propagate through rigid media. |

  ### Different components Expected Modality
  | Sensor Type | Expected Modality of Transducer |
  |-------------|-------------------------------|
  | transducers | Depending on the sensor type |
  |filters; ADCs; amplifiers;circuits| Electrical |
...
4. Try to comprehensively infer all possible vulnerabilities
for example,The aliasing effects of ADCs and other electronic components are mapped to the "Electrical-Frequency Out-of-Range vulnerability".
...
| 2 | Electromagnetic Cross-Field | Antenna Effect｜Interconnect | EM radiation couples into PCB traces acting as antennas. | Attack modality differs from the component's expected modality. |
...
3. For each mechanism, determine:
    1. **Expected Modality** of the component  
      Different components Expected Modality
      | Sensor Type | Expected Modality of Transducer |
      |-------------|-------------------------------|
    ...
    3. Use the taxonomy rules to determine the vulnerability name.
    4. Try to comprehensively infer all possible vulnerabilities
    for example,The aliasing effects of ADCs and other electronic components are mapped to the "Electrical-Frequency Out-of-Range vulnerability".Resonance effects in mechanical transducers are mapped to "Acoustic Cross-Field vulnerability".
Format:
...
2. You may leverage the **Reasoning-Related Knowledge** listed below.

## Reasoning-Related Knowledge (Non-exhaustive; you may extend based on your expertise)
1. Resonance Effect
- attack signal modality: Acoustic
- Fundamental resonance: $f_{attack} \approx f_r$ or $f_{attack} \approx \frac{f_r}{3}$ ($f_{attack}$: attack frequency, $f_r$: mechanical resonant frequency)
- Frequency sweep step limit: $< \frac{f_r}{Q}$ ($Q$: quality factor of the sensitive element)
2. Saturation Effect
- attack signal modality: Acoustic/ Optical/ electromagnetic
- Amplitude threshold: $A_{attack} > x_{max}$ ($A_{attack}$: attack signal amplitude, $x_{max}$: maximum linear input of the sensor/amplifier)
3. Nonlinearity Effect
- attack signal modality: Acoustic/electromagnetic
- Attack signal generation: $s(t) = A[1+m(t)]\cos(2\pi f_c t)$ ($s(t)$: combined attack signal, $A$: amplitude, $m(t)$: modulation signal, $f_c$: carrier frequency, $t$: time)
4. Non-ideal Cutoff
- attack signal modality: Acoustic/ electromagnetic
Frequency selection: $f_{attack} > f_{cutoff}$ ($f_{cutoff}$: filter cutoff frequency)
Power compensation requirement: $H(f_{attack})$ ($H(f)$: filter attenuation response at the given frequency)
5. Aliasing Effect
- attack signal modality: Acoustic/ electromagnetic
- Folding frequency: $f_{alias} = |f_{attack} - k \cdot f_s|$ ($f_{alias}$: resulting low-frequency alias, $k$: an integer, $f_s$: system sampling rate)
6. Photoacoustic Effect
- attack signal modality: Optical
Modulation matching: $f_{modulation} = f_{target}$ ($f_{modulation}$: laser amplitude-modulation frequency, $f_{target}$: target audio or control signal frequency)
7. Photoelectric Effect
- attack signal modality: Optical
Energy threshold: $E = \frac{hc}{\lambda}$ ($E$: photon energy, $h$: Planck's constant, $c$: speed of light, $\lambda$: laser wavelength)
8. Antenna Effect
- attack signal modality: electromagnetic
Efficient coupling: $L \approx \frac{\lambda}{4}$ ($L$: trace or pin length, $\lambda$: wavelength of the electromagnetic interference)
...
* **Datasheet Parameters:** Mechanical resonant frequencies
  ...
  | **Z-Axis** | 24  | 27  | 30  | kHz  |
* **Attack Signal Modality:** Acoustic
* **Attack Type:** Acoustic cross-field
...
* **Attack Type:** Optical amplitude ou-of range
...
* **Attack Signal Modality:** Electromagnetic
* **Attack Type:** Electromagnetic cross-field
...
prompt4_2="""
**Note**:
- You cannot use electric!！！please use sound, light or electromagnetic signals to detect Electrical-Amplitude Out-of-Range 、Electrical-Frequency Out-of-Range and etc
- The attack signals that can ONLY be sound, light, or electromagnetic signals! You cannot use electric!！！These signals can indirectly cause changes in signals such as voltage. 
- Frequency of electromagnetic signals MUST be within the range of **600 MHz to 6000 MHz**.
- Strictly adhere to the attack signal mode and parameter inference methods in Reasoning-Related Knowledge.Please Match the Mechanism
...
- Prioritizing accuracy, please be as precise as possible.
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| 1 | Vulnerability Name 1 (plan1)| [List of Equipment, marked by number] | List signal modality and all required parameters；the more precise, the better;You cannot use electric!！！you can only use sound, light, or electromagnetic signals|1. [Step One]<br>2. [Step Two]<br>... | [Describe the expected test result] |[Provide reasoning for parameter inference，Provide real data and references as support]|
| 2 | Vulnerability Name 1 (plan2)| [List of Equipment, marked by number] | List signal modality and all required parameters；the more precise, the better;You cannot use electric!！！you can only use sound, light, or electromagnetic signals|1. [Step One]<br>2. [Step Two]<br>... | [Describe the expected test result] |[Provide reasoning for parameter inference，Provide real data and references as support]|
| 3 | Vulnerability Name 2 | [List of Equipment, marked by number] | List signal modality and all required parameters；the more precise, the better;You cannot use electric!！！you can only use sound, light, or electromagnetic signals|1. [Step One]<br>2. [Step Two]<br>... | [Describe the expected test result] |[Provide reasoning for parameter inference，Provide real data and references as support]|
